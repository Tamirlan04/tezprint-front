$(document).ready(function() {
      $(".team-photo").click(function() {
        const info = $(this).siblings(".team-info");
        info.fadeToggle(1000); // плавное появление / исчезновение за 1 секунду
      });
    });
  $(".team-photo").click(function() {
    const imgSrc = $(this).attr("src");
    const caption = $(this).siblings(".team-info").text() || $(this).siblings(".team-name").text();
    $("#popupImg").attr("src", imgSrc);
    $("#popupText").text(caption);
    $("#popup").fadeIn(400);
  });

  $(".close, #popup").click(function(e) {
    if (e.target !== $("#popupImg")[0]) {
      $("#popup").fadeOut(300);
    }
  });
